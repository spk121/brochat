...
require_once ABS_PRIVATE_DIR . '/auth.php';
require_once ABS_PRIVATE_DIR . '/db.php';
...
$stmt = $db->prepare("
    SELECT 'note' AS type, notes.id, notes.timestamp, users.username, notes.filename, 
           NULL AS pic1_filename, NULL AS pic2_filename, NULL AS pic3_filename,
        NULL AS pic4_filename
    FROM notes 
    JOIN users ON notes.author_id = users.id 
    WHERE notes.status = 'visible'
    UNION
    SELECT 'snap' AS type, pics.id, pics.timestamp, users.username, pics.caption_filename, 
        pics.pic1_filename, pics.pic2_filename, pics.pic3_filename, pics.pic4_filename
    FROM pics 
